...
#pragma mark Exchange listener methods

- (void)player:(GKPlayer *)player receivedExchangeRequest:(GKTurnBasedExchange *)exchange forMatch:(GKTurnBasedMatch *)match
{
  NSLog(@"Received exchange request %@", exchange);
  self.currentExchangeID = exchange.exchangeID;
  self.lastExhangeMatch = match;
  
  NSString *exchangeString = [[NSString alloc] initWithData:exchange.data encoding:NSUTF8StringEncoding];
  UIAlertView *av = [[UIAlertView alloc] initWithTitle:@"You've Been Taunted" message:exchangeString delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil];
...
{
  NSLog(@"Received exchange cancellation %@", exchange);
}
...
{
  NSLog(@"Received exchange replies %@", replies);
  //1
  GKTurnBasedExchangeReply *firstReply = [replies firstObject];
  NSString *message = [[NSString alloc] initWithData:firstReply.data encoding:NSUTF8StringEncoding];
  UIAlertView *av = [[UIAlertView alloc] initWithTitle:firstReply.message message:message delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
  [av show];
  //2
  __weak typeof(match) weakMatch = match;
  __weak typeof(exchange) weakExchange = exchange;
  [match loadMatchDataWithCompletionHandler:^(NSData *matchData, NSError *error) {
    NSMutableArray *completedExchanges = [NSMutableArray arrayWithObject:weakExchange];
    for (GKTurnBasedExchange *matchExchange in weakMatch.exchanges) {
      if (exchange.status == GKTurnBasedExchangeStatusComplete) {
        [completedExchanges addObject:matchExchange];
      }
    }
    //3
    [weakMatch saveMergedMatchData:matchData withResolvedExchanges:completedExchanges completionHandler:^(NSError *error) {
      if (error) {
        NSLog(@"Error resolving exchanages %@", error);
      }
    }];
  }];
}
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
  ...
  if ([inputString length]) {
    ...
    //4
    for (GKTurnBasedExchange *exchange in self.lastExhangeMatch.exchanges) {
      if ([exchange.exchangeID isEqualToString:self.currentExchangeID]) {
        //5
        [exchange replyWithLocalizableMessageKey:@"Taunt Reply" arguments:nil data:inputStringData  completionHandler:^(NSError *error) {
          if (error) {
            NSLog(@"error %@", error);
          }
        }];
