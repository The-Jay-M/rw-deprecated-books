...
- (void)checkForEnding:(NSData *)matchData {
    if ([matchData length]) {
        self.statusLabel.text = [NSString stringWithFormat:
                                 @"%@, only about %ld letters left",
                                 self.statusLabel.text, (unsigned long)(4000 - [matchData length])];
    }
}
...
-(void)enterNewGame:(GKTurnBasedMatch *)match {
  ...
  self.statusLabel.text = [NSString stringWithFormat:
                           @"Player %ld's Turn (that's you)", (long)playerNum];
...
-(void)takeTurn:(GKTurnBasedMatch *)match {
  ...
  [match loadMatchDataWithCompletionHandler:^(NSData *matchData, NSError *error) {
    if ([matchData bytes]) {
      ...
      NSMutableArray *completedExchanges = [NSMutableArray array];
      for (GKTurnBasedExchange *exchange in match.exchanges) {
        if (exchange.status == GKTurnBasedExchangeStatusComplete) {
          [completedExchanges addObject:exchange];
        }
      }
      if ([completedExchanges count]) {
        [match saveMergedMatchData:matchData withResolvedExchanges:completedExchanges completionHandler:^(NSError *error) {
          if (error) {
...
-(void)layoutMatch:(GKTurnBasedMatch *)match {
  ...
  if (match.status == GKTurnBasedMatchStatusEnded) {
    ...
    statusString = [NSString stringWithFormat:
                    @"Player %ld's Turn", (unsigned long)playerNum];
  }
