...
-(void)update:(ccTime)dt
{
    ...
    if (js == kJumpButtonOn) {
        if ((self.characterState == kStateJumping || self.characterState == kStateFalling) && jumpReset && self.canDoubleJump) {
...
-(void)changeState:(CharacterStates)newState {
    ...
    switch (newState) {
        ...
        case kStateWallSliding:
            ...
            break;
        case kStateJumping:
            [[SimpleAudioEngine sharedEngine] playEffect:@"jump1.wav"];
            action = [CCAnimate actionWithAnimation:jumpUpAnim];
            break;
        case kStateDoubleJumping:
            [[SimpleAudioEngine sharedEngine] playEffect:@"jump2.wav"];
            [self setDisplayFrame:[[CCSpriteFrameCache sharedSpriteFrameCache] spriteFrameByName:@"Player10.png"]];
            break;
        case kStateDead:
            [[SimpleAudioEngine sharedEngine] playEffect:@"player_die.wav"];
